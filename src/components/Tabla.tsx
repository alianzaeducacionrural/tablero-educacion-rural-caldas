...
/** Tabla ordenable: es el equivalente accesible de cada gráfico y permite descargar los datos. */
  ...
  return (
    <div>
          ...
          <thead className="sticky top-0 bg-surface">
            <tr>
              {columnas.map((c) => {
                ...
                return (
                  <th key={c.clave} scope="col" aria-sort={activa ? (orden!.asc ? 'ascending' : 'descending') : 'none'} className={`border-b border-line px-3 py-2 font-medium text-ink2 ${c.tipo && c.tipo !== 'texto' ? 'text-right' : 'text-left'}`}>
                    <button type="button" onClick={() => setOrden({ clave: c.clave, asc: activa ? !orden!.asc : c.tipo === 'texto' || !c.tipo })} className="hover:text-ink">
                      {c.titulo}
          ...
          <tbody>
            {ordenadas.map((f, i) => (
              <tr key={i} className="border-b border-line last:border-0 hover:bg-wash">
                {columnas.map((c) => (
                  <td key={c.clave} className={`px-3 py-1.5 ${c.tipo && c.tipo !== 'texto' ? 'tabular text-right' : ''}`}>
      ...
      <div className="mt-2 flex items-center justify-between text-xs text-muted">
        ...
        <button
          type="button"
          onClick={() => descargarCsv(`${archivo}.csv`, columnas.map((c) => c.titulo), ordenadas.map((f) => columnas.map((c) => f[c.clave] ?? '')))}
          className="rounded-md px-2 py-1 text-accentink hover:bg-wash"
        >
          Descargar CSV
        </button>
      </div>
